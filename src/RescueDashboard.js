...
export default function RescueDashboard() {
  ...
  useEffect(() => {
    axios.get(`${SOCKET_URL}/api/alerts`)
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: 'Active Signals', val: tickets.length, icon: <AlertOctagon size={24}/>, color: 'text-rose-500', border: 'border-rose-500/20', shadow: 'shadow-[0_0_20px_rgba(225,29,72,0.1)]' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-[#0a0a0a]/80 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden h-[550px] relative shadow-2xl">
          ...
          <MapContainer center={mapCenter} zoom={13} zoomControl={false} scrollWheelZoom={false} className="w-full h-full">
            ...
            />
            {tickets.map(t => (
              <Marker key={t._id} position={[t.lat, t.lng]} icon={createIcon(typeColors[t.type])}>
                <Popup>
                  <div className="font-sans">
                    <strong className="block text-base font-bold mb-1 tracking-wider text-slate-100">{t.ticketId}</strong>
        ...
        <div className="bg-[#0a0a0a]/80 backdrop-blur-xl border border-white/10 rounded-2xl flex flex-col h-[550px] shadow-2xl">
          ...
          <div className="overflow-y-auto p-4 space-y-4 flex-1 custom-scrollbar">
            {tickets.map((t) => (
              <div key={t._id} className="bg-[#050505] border border-white/5 p-5 rounded-xl hover:border-white/20 transition-all duration-300">
                <div className="flex justify-between items-start mb-3">
                  <span className="font-mono font-bold tracking-wider text-slate-200 text-sm">{t.ticketId}</span>
                ...
                <div className="flex justify-between items-center">
                  <span className="text-xs text-slate-500 font-mono font-medium">{formatTime(t.timestamp)}</span>
